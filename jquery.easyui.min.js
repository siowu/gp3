        ...
        for (var _b2 = 0; _b2 < aa.length; _b2++) {
            ...
            if (_b3 == "list") {
                if (_af.pageList.length > 0) {
                    ...
                    var ps = $("<div class=\"pagination-page-input-container\"><input type=\"text\" class=\"pagination-page-input\" /><i class=\"fa fa-caret-down pagination-page-icon\" onclick=\"toggleDropdown()\"></i> <ul class=\"dropdown-options\"><li data-value='50'>50</li><li data-value='100'>100</li><li data-value='200'>200</li><li data-value='500'>500</li><li data-value='1000'>1000</li></ul></div>");
                    // var ps = $("<div class=\"pagination-page-input-container\"><input type=\"text\" value=\"50\" class=\"pagination-page-input\" /><i class=\"fa fa-caret-down pagination-page-icon\" onclick=\"toggleDropdown()\"></i> <ul class=\"dropdown-options\"><li data-value='50'>50</li><li data-value='100'>100</li><li data-value='200'>200</li><li data-value='500'>500</li><li data-value='1000'>1000</li></ul></div>");
                }
        ...
        render: function (_1c7, ul, data) {
            ...
            function _1cd(_1ce, _1cf) {
                ...
                for (var i = 0; i < _1cf.length; i++) {
                    ...
                    // var tree_indent_level = _1ce;
                    for (var j = 0; j < _1ce; j++) {
                        cc.push("<span class=\"tree-indent\"></span>");
                    }
                    // if (tree_indent_level > 0) {
                    //     cc.push("<span class='tree-indent-" + tree_indent_level +"'></span>");
                    // }
                    ...
                    cc.push("<span class=\"tree-title\" title='" + opts.formatter.call(_1c7, item) + "'>" + opts.formatter.call(_1c7, item) + "</span>");
                    cc.push("</div>");
                    if (item.children && item.children.length) {
...
(function ($) {
    ...
    function _395(_396) {
        ...
        $("<div class=\"tabs-header\">" + "<div class=\"tabs-scroller-right\"></div>" + "<div class=\"tabs-wrap\">" + "<ul class=\"tabs\"></ul>" + "</div>" + "</div>").prependTo(_396);
...
(function ($) {
    ...
    function show(_727) {
        ...
        if (opts.current == "Invalid Date") {
            // 修正datetimebox为空时，无法唤起日期选择器的问题
...
(function ($) {
    ...
    function _aee(_aef, _af0) {
        ...
        hit.removeClass("tree-collapsed tree-collapsed-hover").addClass("tree-expanded");
        // 打上标记，用于隐藏权限管理页面，tree展开时出现tree-folder-open样式
        // hit.next().addClass("tree-folder-open");
...
(function ($) {
    $.extend($.fn.validatebox.defaults.rules, {
        ...
        daterange: {
            validator: function (value, param) {
                const startDate = $(param[0]).datebox('getValue');
...
(function ($) {
    $.fn.extend({
        searchableCombogrid: function (options, ...args) {
            const defaults = {
                showHeader: true,
                placeholder: 'Please enter keywords', //搜索提示
                filterDelay: 300,    // 输入延迟过滤时间
                panelWidth: 340,
                panelHeight: 360,
                scrollbarSize: 0,
                idField: 'value',
                ...
                fitColumns: false,
                columns: [[
                    { field: 'value', title: '', width: 100 },
                    { field: 'label', title: '', width: 240 },
                ]],
                // fixed: false,
                // fitColumns: true,
            };
            ...
            return this.each(function () {
                const $input = $(this);
                const inputId = $input.attr('id') || Math.random().toString(36).substring(2, 9);
                const comboGrid = $('#' + inputId);
                // 初始化 combogrid
                comboGrid.combogrid($.extend({}, settings, {
                    ...
                    icons: [
                        {
                            iconCls: 'icon-clear', handler: function (e) {
                                ...
                                // toggleCombogridClearIcon('#' + inputId, [])
                            }
                        }
                    ],
                    onChange: function (newValue, oldValue) {
                        if (oldValue.length > 0 && newValue.length == 0) {
                            ...
                            toggleCombogridClearIcon('#' + inputId, newValue)
                        }
                    },
                    onSelect: settings?.handleSelect || function (index, row) {

                        const multiple = settings.multiple || false

                        if (!multiple || row[settings.idField] == 'All') {
                            comboGrid.combogrid('setValue', row[settings.idField])
                            // toggleCombogridClearIcon('#' + inputId, comboGrid.combogrid('getValues'))
                            return;
                        }
                        // if () {
                        //     comboGrid.combogrid('setValue', 'All')
                        //     toggleCombogridClearIcon('#' + inputId, comboGrid.combogrid('getValues'))
                        //     return;
                        // }

                        var values = comboGrid.combogrid('getValues')
                        values = values.filter(function (value) {
                            return value != 'All'
                        })
                        comboGrid.combogrid('setValues', values)
                        // toggleCombogridClearIcon('#' + inputId, comboGrid.combogrid('getValues'))
                        return;
                    },

                    onShowPanel: settings?.handleShowPanel || function () {

                        console.log('onshowpanel', settings.panelWidth)
                        const grid = comboGrid.combogrid('grid');

                        const searchboxName = inputId + '-searchbox';
                        const searchBox = $('<div class="searchable-combogrid-filter" id="' + searchboxName + '" style="padding:5px;width:' + (settings.panelWidth - 60) + 'px">' +
                            '<input type="text" placeholder="' + settings.placeholder + '" style="width:100%; box-sizing:border-box;padding:4px;font-size:12px" /></div>');

                        // 插入搜索框到面板顶部
                        const panel = comboGrid.combogrid('panel');
                        const tr = panel.find('tr.datagrid-header-row')
                        if (tr.eq(1).find('input').length > 0) {
                            tr.eq(1).find('input').val('')
                        }
                        if (tr.eq(1).find('.searchable-combogrid-filter').length === 0) {
                            $(".datagrid-header-check").html("");
                            tr.eq(1).html(searchBox)
                            ...
                            //         const data = grid.datagrid('getData').rows;
                            //         const _index = data.findIndex(function (item) {
                            //             // console.log('item', item, settings.idField, item[settings.idField])
                            //             return item[settings.idField].toUpperCase().indexOf(value.toUpperCase()) > -1
                            //         });
                            //         if (_index > -1) {
                            //             grid.datagrid('scrollTo', _index);
                            //             grid.datagrid('highlightRow', _index);
                            //         }
                            //     }, settings.filterDelay);
                            // })

                            searchInput.off('input.scroll').on('input.scroll', function (e) {
                                clearTimeout(timer);
                                timer = setTimeout(() => {
                                    ...
                                    const data = grid.datagrid('getData').rows;
                                    const _index = data.findIndex(function (item) {
                                        // console.log('item', item[settings.idField], value.toUpperCase(), item[settings.idField].toUpperCase().indexOf(value.toUpperCase()))
                                        return item[settings.idField].toUpperCase().indexOf(value.toUpperCase()) > -1
                                    });
                                    if (_index > -1) {
                                        const body = grid.datagrid('getPanel').find('.datagrid-body')
                                        const rowHeight = body.find('tr:first').height();
                                        body.find(1).scrollTop(_index * rowHeight);
                                        grid.datagrid('highlightRow', _index)
                                    }
                                }, settings.filterDelay)
                            })
                            ...
                            searchInput.on('keydown', function (e) {
                                if (e.keyCode === 13) {
                                    ...
                                    const multiple = settings.multiple || false
                                    const data = grid.datagrid('getData').rows;
                                    var filter = data.find(function (item) {
                                        return item[settings.idField].toUpperCase().indexOf(value.toUpperCase()) > -1
                                    })
                                    if (!filter) {
                                        $(this).val('')
                                    ...
                                    if (!multiple || filter[settings.idField] == 'All') {
                                        // comboGrid.combogrid('setValue', filter[settings.idField]);
                                        // toggleCombogridClearIcon('#' + inputId, comboGrid.combogrid('getValues'))
                                        // 将setValue改为selectRow，好处是，会执行onselect中的方法
                                        const _index = data.findIndex(function (item) { return item[settings.idField] == filter[settings.idField] })
                                        if (_index != -1) {
                                            comboGrid.combogrid('grid').datagrid('selectRow', _index)
                                    ...
                                    if (values.indexOf(filter[settings.idField]) === -1) {
                                        values.push(filter[settings.idField])
                                        comboGrid.combogrid('setValues', values);
                                        // toggleCombogridClearIcon('#' + inputId, comboGrid.combogrid('getValues'))
                                        $(this).val('')
                                        return;
